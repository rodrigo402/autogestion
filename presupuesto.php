...
<?php
		...
		} else if (isset($_GET['accion']) == 'presupuesto') {
			
			$idc        = $_POST['idc'];
			$descipcion = $_POST['descipcion'];
			$resultado = guardarPresupuesto($idc, $descipcion);
			if ($resultado) {
				unset($_SESSION['presupuesto']);	
				echo "<script> alert('presupuesto guardado'); </script>";
			}
			
		}
		
	?>
<div class="titulo"><h1>Presupuesto</h1></div>	
<nav>
	<ul>
...
<div id="flotante2" style="display:none;">
		<div class="separacion">
			<form action="presupuesto.php?accion=producto" method="POST">
				<h1>PRODUCTOS</h1>
				<label>NOMBRE:</label>
...
<div class="izquierdo">
			<form action="presupuesto.php?accion=presupuesto" method="POST">
				<h1>CLIENTE</h1>
			<label>NOMBRE:</label>
			...
			<input type="text" name="descipcion" placeholder="descipcion"class="form-control"><br>
			<center><button  class="btn btn-success" id="btn">GUARDAR PRESUPUESTO</button><button type="button" name="btn" class="btn" onclick="imprimir()">IMPRIMIR</button></center>
			</form>
</div>
...
<?php 
	if (isset($_GET['accion']) && $_GET['accion'] == 'producto') {	
			...
			$_SESSION['presupuesto'][] = $producto;
		   for ($i = 0; $i < count($_SESSION['presupuesto']); $i++) {
			   echo "<tr>";
			   echo "<td>" . "</td>";
			   echo "<td>" . $_SESSION['presupuesto'][$i]['idp'] . "</td>";
			   echo "<td>" . $_SESSION['presupuesto'][$i]['tipo'] . "</td>";
			   echo "<td>" . $_SESSION['presupuesto'][$i]['cantidad'] . "</td>";
			   echo "<td>" . $_SESSION['presupuesto'][$i]['MTS'] . "</td>";
			   echo "<td>" . $_SESSION['presupuesto'][$i]['precio'] . "</td>";
			   echo "<td>" . "$". $_SESSION['presupuesto'][$i]['total'] . "</td>";
							$suma += $_SESSION['presupuesto'][$i]['cantidad'] * 
									 $_SESSION['presupuesto'][$i]['MTS'] * 
									 $_SESSION['presupuesto'][$i]['precio'];
			   echo "</tr>";			   
		   }
				echo "<tr>";
				echo "<td>" . "</td>";
